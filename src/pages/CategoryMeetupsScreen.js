...
const CategoryMeetupsScreen = ({ navigation }) => {
    ...
    const fetchPosts = useCallback(async (pageNum = 0, isRefreshing = false) => {
        try {
            ...
            console.error('Error fetching meetup posts:', error);
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    if (loading && items.length === 0) {
        return (
            <SafeAreaView style={styles.container} edges={['top']}>
                <Header showCategoryTabs activeCategory="CategoryMeetups" />
...
export default CategoryMeetupsScreen;
